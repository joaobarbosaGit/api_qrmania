...
class QrCodeFidelidadeRepository implements IQrCodeFidelidadeRepository {
    ...
    async listAllQrCodeFidelidadeByUser(idlojista:number): Promise<QrCodeFidelidade[]> {
        ...
        .leftJoinAndSelect("qrcode_fidelidade.qrcode_fidelidade_produtos","qrcode_fidelidade_produtos")
        .where("qrcode_fidelidade.idlojista = :idlojista", {idlojista: idlojista})
        .andWhere("qrcode_fidelidade.exibir_no_aplicativo = :exibir_no_aplicativo", {exibir_no_aplicativo: "SIM"})
        .getMany();
