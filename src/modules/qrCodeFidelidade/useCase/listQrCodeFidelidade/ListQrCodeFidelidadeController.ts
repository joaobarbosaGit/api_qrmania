...
class ListQrCodeFidelidadeController {
  ...
  async handle(request: Request, response: Response): Promise<Response> {
    ...
    const listQrCodeFidelidadeUseCase = container.resolve(ListQrCodeFidelidadeUseCase);

    const qrcode_fidelidade = await listQrCodeFidelidadeUseCase.execute({idlojista});
